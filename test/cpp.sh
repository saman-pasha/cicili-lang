...
echo "-- M6, in steps: C++ that is C with names, classes, virtual -- built and run (test/cpp/run/NAME.cpp against NAME.expect)"
...
for pair in "control:try" "templates:template"; do
  n=${pair%%:*}; what=${pair#*:}
