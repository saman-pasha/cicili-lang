/* the same tree in plain C, for clang -O3: the B-tree of test/c/run/btree.c at minimum degree 6 (up to 11 keys, 12
   ...
   by clang -O3; the free written by hand, recursive. */
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
static node *new_node(int leaf) { node *x = calloc(1, sizeof(node)); x->leaf = leaf; return x; }
static void move_upper(node *y, node *z) {            /* y full: keys 6..10 and children 6..11 go to z */
    z->n = 5; z->leaf = y->leaf;
    for (int j = 0; j < 5; j++) z->key[j] = y->key[j + 6];
    if (!y->leaf) for (int j = 0; j < 6; j++) z->C[j] = y->C[j + 6];
    y->n = 5;
}
...
static void drop(node *x) { if (!x) return; if (!x->leaf) for (int i = 0; i <= x->n; i++) drop(x->C[i]); free(x); }
...
int main(int argc, char **argv) {
    ...
    drop(t.root);
    long t3 = now_ms();
    printf("clang -O3    insert %ld ms  search %ld ms  free %ld ms  found %d\n", t1 - t0, t2 - t1, t3 - t2, found);
    return 0;
}
