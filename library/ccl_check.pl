...
ck_fail(Kind, Name, Form) :- nb_getval('$ck_fn', F), nb_getval('$ck_line', L), throw(error(ownership(Kind, Name, Form), where(F, line(L)))).
...
ck_is_owner(St, N) :- ck_state(St, N, _).
...
ck_merge_owners([], _, []).
ck_merge_owners([N-Sa|T], Ob, [N-Sc|T1]) :- ( memberchk(N-Sb, Ob) -> ( Sa == Sb -> Sc = Sa ; Sc = partial ) ; Sc = Sa ), ck_merge_owners(T, Ob, T1).
...
ck_stmt(declaration(L, _, _, Vs), St0, St) :- !, nb_setval('$ck_line', L), ck_decls(Vs, St0, St).
...
ck_stmt(empty, St, St) :- !.
ck_stmt(expr(E), St0, St) :- !, ck_expr(E, St0, St).
ck_stmt(defer(L, _, Body), St0, St) :- !, nb_setval('$ck_line', L), ck_defer(St0, Body, St).
ck_stmt(if(C, T, E), St0, St) :- !,
    ck_expr(C, St0, St1), ck_stmt(T, St1, StT), ( E == none -> StE = St1 ; ck_stmt(E, St1, StE) ), ck_merge(StT, StE, St).
ck_stmt(while(C, S), St0, St) :- !, ck_expr(C, St0, St1), ck_loop(S, St1, St2), ck_expr(C, St2, St).
ck_stmt(do(S, C), St0, St) :- !, ck_loop(S, St0, St1), ck_expr(C, St1, St).
ck_stmt(for(Init, C, Step, S), St0, St) :- !,
    ck_push(St0, St1),
    ...
    ck_scope_end(St4, St).
ck_stmt(return, St0, dead) :- !, ck_exit_all(St0, return).
ck_stmt(return(E), St0, dead) :- !, ck_consume_or_use(E, St0, St1), ck_exit_all(St1, return(E)).
ck_stmt(break, St0, dead) :- !, ck_exit_to_loop(St0, break).
ck_stmt(continue, St0, dead) :- !, ck_exit_to_loop(St0, continue).
ck_stmt(goto(L), St, dead) :- !, ( ck_any_owner(St) -> ck_fail(goto_with_owners, L, goto(L)) ; true ).
ck_stmt(label(_, S), St0, St) :- !, ck_stmt(S, St0, St).
ck_stmt(switch(E, S), St0, St) :- !,
    ck_expr(E, St0, St1),
    ...
    ck_scope_end(St5, St).
ck_stmt(case(_, S), St0, St) :- !, ck_stmt(S, St0, St).
ck_stmt(default(S), St0, St) :- !, ck_stmt(S, St0, St).
ck_stmt(S, _, _) :- ck_fail(not_checked, S, S).
ck_stmts([], St, St).
ck_stmts([S|Ss], St0, St) :- ck_stmt(S, St0, St1), ck_stmts(Ss, St1, St).

ck_switch_items([], _, St, St).
ck_switch_items([case(E, S)|Is], Entry, St0, St) :- !, ck_merge(St0, Entry, St1), ck_switch_items([S|Is], Entry, St1, St).
ck_switch_items([default(S)|Is], Entry, St0, St) :- !, ck_merge(St0, Entry, St1), ck_switch_items([S|Is], Entry, St1, St).
ck_switch_items([S|Is], Entry, St0, St) :- ck_stmt(S, St0, St1), ck_switch_items(Is, Entry, St1, St).
ck_has_default(Is) :- member(default(_), Is), !.
ck_has_default(Is) :- member(case(_, S), Is), ck_has_default([S]), !.

%% declarations: an own variable is an owner from here
ck_decls([], St, St).
ck_decls([var(N, T, Init)|Vs], St0, St) :-
    ( Init == none -> St1 = St0 ; Init = init(Items) -> ck_init_items(Items, St0, St1) ; ck_consume_or_use(Init, St0, St1) ),
    ( ck_own_type(T) -> ck_declare(St1, N, St2) ; St2 = St1 ),
    ck_decls(Vs, St2, St).
ck_init_items([], St, St).
...
ck_leaks([], _).
ck_leaks([N-S|Os], Form) :- ( S == moved -> true ; ck_fail(owner_leaked, N, Form) ), ck_leaks(Os, Form).
...
ck_consume(N, Form, St0, St) :-
    ck_state(St0, N, S), ( S == live -> true ; ck_fail(use_after_move, N, Form) ), ck_set(St0, N, moved, St).

ck_expr(_, dead, dead) :- !.
ck_expr(id(N), St, St) :- !, ( ck_state(St, N, S), S \== live -> ck_fail(use_after_move, N, id(N)) ; true ).
