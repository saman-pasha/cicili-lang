%% cicili-cpp -- library(ccl_syntax): C source, read as a DCG, into an AST.
%%
...
%%   typedef(Line, [var(Name, Type, none) ...])
...
%%         ptr(Quals, Type)   arr(Size, Type)   fn(Ret, Params, Variadic)
...
ccl_unit(Tokens, unit(Items), Rest) :- ccl_seed_typedefs(Env), nb_setval('$ccl_env', Env), nb_setval('$ccl_far', 0), phrase(ccl_externals(Env, Items), Tokens, Rest).
...
ccl_externals(Env0, [I|Is]) --> ccl_external(Env0, Env1, I), !, ccl_set_env(Env1), ccl_externals(Env1, Is).
ccl_externals(_, []) --> [].
...
ccl_external_rest(Env, Env, L, Sto, Base, function(L, Sto, Ret, Name, Params, Var, Body)) -->
    ccl_declarator(Env, Base, Name, Type), { Type = fn(Ret, Params, Var) }, ccl_attrs,
    ccl_compound(Env, Body), !.
ccl_external_rest(Env0, Env, L, Sto, Base, Item) -->
    ...
    ; Env = Env0, Item = declaration(L, Sto, Base, Ds) }.
ccl_external_rest(Env, Env, L, _, Base, declare(L, Base)) --> ccl_p(';').
ccl_declared_names([], []).
...
ccl_gnu_word(W) :- sub_atom(W, 0, 2, _, '__').
...
ccl_gnu_attr --> ccl_id(A), { memberchk(A, ['__extension__', '__inline__', '__inline', '__restrict', '__restrict__', '__volatile__']) }.
...
ccl_struct_body(Env, K, T) --> ccl_id(N), !, ( ccl_p('{'), !, ccl_members(Env, Ms), ccl_p('}'), { T =.. [K, N, Ms] } ; { T =.. [K, N, none] } ).
ccl_struct_body(Env, K, T) --> ccl_p('{'), ccl_members(Env, Ms), ccl_p('}'), { T =.. [K, anon, Ms] }.
...
ccl_param(Env, param(T, N)) --> ccl_decl_specs(Env, param, _, Base), ccl_abstract_or_declarator(Env, Base, N, T).
...
%% ---- statements -------------------------------------------------------------
ccl_compound(Env, block(Items)) --> ccl_p('{'), ccl_block_items(Env, Items), ccl_p('}').
ccl_block_items(Env0, [I|Is]) --> ccl_block_item(Env0, Env1, I), !, ccl_set_env(Env1), ccl_block_items(Env1, Is).
ccl_block_items(_, []) --> [].
...
ccl_block_item(Env0, Env, I) --> ccl_line(L), ccl_decl_specs(Env0, block, Sto, Base), ccl_init_declarators(Env0, Base, Ds), ccl_p(';'), !,
    ...
    ; Env = Env0, I = declaration(L, Sto, Base, Ds) }.
ccl_block_item(Env, Env, declare(L, Base)) --> ccl_line(L), ccl_decl_specs(Env, block, _, Base), ccl_p(';'), !.
...
ccl_statement(_, expr(E)) --> ccl_expr(E), ccl_p(';').

ccl_for_init(Env, decl(Base, Ds)) --> ccl_decl_specs(Env, block, _, Base), ccl_init_declarators(Env, Base, Ds), ccl_p(';'), !.
...
ccl_postfix_(A, E) --> ccl_p('('), !, ccl_args(As), ccl_p(')'), ccl_postfix_(call(A, As), E).
